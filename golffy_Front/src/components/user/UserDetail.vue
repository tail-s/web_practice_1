    ...
    <div class="login-container">
      <section class="login" id="login">
        ...
        <form class="login-form">
          ID <input type="text" class="login-input" :value="id" readonly />
          NickName <input type="text" class="login-input" :value="nick" readonly />
          <div>작성한 게시글 : {{ bCnt }} 개</div>
          <div>작성한 댓글 : {{ cCnt }} 개</div>
...
export default {
  ...
  created() {
    this.$store.dispatch("getbcnt", localStorage.getItem("user_nick"));
    this.$store.dispatch("getccnt", localStorage.getItem("user_nick"));
    console.log("mount");
    if (this.ccnt == undefined) {
      this.ccnt = 0;
    }
  },
  methods: {
